...
# Function to find the machine with the highest repair time
def find_highest_repair_time(data):
    max_time = 0
    machine_with_max_time = None
    
    for row in data:
        time_taken = float(row.get("Time Taken (in hours)", 0))
        if time_taken > max_time:
            max_time = time_taken
            machine_with_max_time = row.get("Machine Name", "")
    
    return machine_with_max_time, max_time

# Function to calculate total production loss for CNC machines
def calculate_total_production_loss_cnc(data):
    total_loss = 0
    
    for row in data:
        if "CNC" in row.get("Machine Name", ""):
            production_loss = float(row.get("Production Loss ()", 0))
            total_loss += production_loss
    
    return total_loss
    
def get_machine_by_id_or_name(data, machine_id_or_name):
    for row in data:
        if row.get("Machine Name", "").lower() == machine_id_or_name.lower() or row.get("ID", "").lower() == machine_id_or_name.lower():
            return row
    return None   

# Enhanced function to detect intent from user query
def detect_intent(user_query):
    user_query = user_query.lower()
    
    # Intent: Count CNC machines
    if re.search(r'\bcnc\b', user_query) and re.search(r'\b(count|number|how many)\b', user_query):
        return "count_cnc_machines"
    
    # Intent: Count total machines
    if re.search(r'\b(total|number|how many)\b', user_query) and re.search(r'\bmachine\b', user_query):
        return "count_total_machines"
    
    # Intent: Machines repaired before a specific year
    if re.search(r'\brepaired (before|till)\b', user_query):
        return "machines_repaired_before_year"
    
    # Intent: Get machine description by ID
    if re.search(r'\bdescription\b', user_query) and re.search(r'\bmachine\b', user_query):
        return "get_machine_description_by_id"
    
    # Intent: Machines repaired by a specific technician
    if re.search(r'\brepaired by\b', user_query) or re.search(r'\btechnician\b', user_query):
        return "get_machines_repaired_by_technician"
    
    # Intent: Find machine with highest repair time
    if re.search(r'\bhighest (time|hour)\b', user_query) and re.search(r'\brepair\b', user_query):
        return "highest_repair_time"
    
    # Intent: Calculate total production loss for CNC machines
    if re.search(r'\bproduction loss\b', user_query) and re.search(r'\bcnc\b', user_query):
        return "total_production_loss_cnc"
    
    # Intent: Find machine by name or ID
    if re.search(r'\b(machine|mm)\b', user_query) and re.search(r'\b\d{3}\b', user_query):
        return "get_machine_by_id_or_name"
...
async def main(message):
    ...
    elif intent == "highest_repair_time":
        machine, time = find_highest_repair_time(data)
        response = f"The machine with the highest repair time is {machine} with {time} hours."
    elif intent == "total_production_loss_cnc":
        total_loss = calculate_total_production_loss_cnc(data)
        response = f"The total production loss due to CNC machine repairs is {total_loss}%."
    elif intent == "get_machine_by_id_or_name":
        machine_id_or_name = user_query.lower().split("machine")[1].strip()
        result = get_machine_by_id_or_name(data, machine_id_or_name)
        if result:
            response = (
                f"**🔧 Machine Name:** {result.get('Machine Name', 'N/A')}\n\n"
                f"**🔧 Problem:** {result.get('Issue Description', 'N/A')}\n\n"
            ...
            response = f"No machine found with ID or name {machine_id_or_name}."
    else:
